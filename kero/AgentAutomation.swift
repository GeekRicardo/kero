...
final class KeroAgentObservationState {
    ...
    /// Set once this agent has reported a lifecycle event of its own. Only
    /// such an agent is notified about from the event itself; the rest are
    /// recognized as processes and have no turn to announce.
    var hasNativeLifecycle = false
    /// Set once this agent has reported the *start* of a turn. Until then a
    /// turn Kero submitted through `+agent` is the only evidence a turn ever
    /// happened; afterwards the provider is the only thing that decides what
    /// a turn is.
...
extension TerminalSession {
    ...
    func reportAutomationAgent(
        ...
        // merely opening a conversation and looking away must not announce
        // that something finished. A provider that reports idle and nothing
        // else has no start to go by, so a turn Kero itself submitted through
        // `+agent` still counts as one.
    ...
    /// that fact with focus and with what the pane is already presenting.
    /// Tying the notification to the status is what let a pane announce a
    /// completion nobody had asked for.
    ///
    ...
    private func updateAutomationAgentStatus(
        ...
        // event itself — see `notifyAgentEvent`. This guard keeps the same turn
        // from being announced twice, once as an event and once as the status
        // that event produced.
